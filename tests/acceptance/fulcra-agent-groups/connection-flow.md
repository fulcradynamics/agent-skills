...
Preconditions and runtime: three authorized test accounts; the creator and one joiner run through CLI-capable agents, and one member reads through an MCP-only agent after a CLI join. Verify each authenticated account ID before mutations.
