...
Data is recorded by sending a `POST` request to the Fulcra Ingest API. The endpoint supports batch ingestion using a JSON array of flattened record objects.

```bash
curl -i -X POST \
  -H "Authorization: Bearer $(uv tool run fulcra-api auth print-access-token)" \
  -H "Content-Type: application/json" \
  -d '[ { ... json payload 1 ... }, { ... json payload 2 ... } ]' \
...
The JSON payload for each record is a single, flattened object.
...
curl -i -X POST \
  ...
  -d '[
    {
      "record_id": "<UUID_OF_RECORD_TO_DELETE>",
...
[
  {
    ...
    "note": "Stranger Things: Season 1: Chapter One"
  }
]
```
...
[
  {
    ...
    "note": "Keychron Mechanical Keyboard",
    "value": 89.99
  }
]
```
...
[
  {
    ...
    "note": "Completed Morning Meditation",
    "value": true
  }
]
```
