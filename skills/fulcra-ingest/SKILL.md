...
3. **Adjust Existing Data:** You can adjust the tag scheme, values, or anything about existing data. To do this, delete the old records via the `fulcra-api delete` command, update the schema (creating a new data type if necessary), and re-ingest the data. **Security Requirement:** Deleting records is a destructive operation. You MUST prominently surface this risk and require explicit user confirmation of the exact records affected before running the `fulcra-api delete` command. You can re-import previously archived files from `ingest/_meta/archive/artifact/` to process the new tags. Before modifying or re-processing existing records, confirm the plan with the user.
...
0. **The Fetcher (API/CLI Extraction)** *(If applicable)*
   - You may inform the user that you can process manual file uploads or extract data from accessible APIs, local services, and CLI tools on their behalf. 
   - **Security Requirement:** You must receive explicit user consent before fetching data from any external API, local service, or CLI tool. Confirm the exact source, destination, frequency, and privacy implications with the user.
...
3. **Cleanup & Archive**
   ...
   - **Security Requirement:** Explicitly request user confirmation before running the `fulcra-api file delete` command to remove the original file from the `ingest/` directory (e.g., `uvx fulcra-api file delete ingest/NetflixViewingHistory.csv`).
   - Finalize the process by updating the `source_map.md` in memory. Before overwriting the remote file, ensure you have downloaded the latest version to prevent wiping out concurrent changes, and ask the user to confirm the metadata modification if this is an interactive session. Then, upload it back to Fulcra, as instructed in the source mapping reference.
