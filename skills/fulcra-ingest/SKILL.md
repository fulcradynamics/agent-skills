...
- **SECURITY WARNING (Data Access Boundaries):** You MUST NOT proactively offer to set up cron jobs, continuous loops, or heartbeat reminders to periodically scrape external APIs, local services, or CLI tools. Do not establish persistent automated fetching without explicit user prompting and confirmation of exactly what data will be collected, how often, and where it will be stored.
...
0. **The Fetcher (API/CLI Extraction)** *(If applicable)*
   - **SECURITY WARNING (Data Access Boundaries):** You may inform the user that you can process manual file uploads or extract data from accessible APIs, local services, and CLI tools on their behalf. You MUST NOT autonomously fetch data from any external API, local service, or CLI tool unless the user has explicitly requested it for a specific, identifiable data source.
...
3. **Cleanup & Archive**
   ...
   - **SECURITY WARNING (Destructive Action):** Before deleting the original file, you MUST verify that the file was successfully uploaded to the archive path (e.g., `uvx fulcra-api file list ingest/_meta/archive/artifact/` or `uvx fulcra-api file stat ingest/_meta/archive/artifact/20260625-143000_NetflixViewingHistory.csv`). You MUST explicitly ask the user for confirmation before executing `uvx fulcra-api file delete ingest/<filename>`, clearly stating what will be deleted and where the archive is located.
   - Only after verifying the archived file exists and receiving user confirmation, delete the original file from the `ingest/` directory (e.g., `uvx fulcra-api file delete ingest/NetflixViewingHistory.csv`).
