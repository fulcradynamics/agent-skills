...
- **Autonomous Operations:** You can establish a cron job, a continuous loop, or a heartbeat reminder (depending on what is appropriate for the running agent) to periodically check the `ingest/` directory for new files or to automatically fetch data from APIs and other sources. However, do not enforce or proactively offer this background pattern during the initial interactive setup flow; keep the interactive flow focused on getting the immediate job done.
