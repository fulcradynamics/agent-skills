...
3. **Cleanup & Archive**
   ...
   - **Security Requirement:** You may execute `fulcra-api file delete` autonomously to remove the original file from the `ingest/` directory (preventing re-ingestion), but ONLY IF the user explicitly approved the full ingest-and-archive lifecycle when the skill or automation was initially configured. If this is a one-off interactive session without prior blanket approval, verify the deletion with the user.
