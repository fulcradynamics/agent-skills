...
3. **Adjust Existing Data:** You can adjust the tag scheme, values, or anything about existing data. To do this, delete the old records via the `fulcra-api delete` command, update the schema (creating a new data type if necessary), and re-ingest the data. Deleting records is a destructive operation. You must prominently surface this risk and require explicit user confirmation of the exact records affected before running the `fulcra-api delete` command. You can re-import previously archived files from `ingest/_meta/archive/artifact/` to process the new tags. Before modifying or re-processing existing records, confirm the plan with the user.
...
0. **The Fetcher (API/CLI Extraction)** *(If applicable)*
   ...
   - You must receive explicit user consent before fetching data from any external API, local service, or CLI tool. Confirm the exact source, destination, frequency, and privacy implications with the user.
