---
name: fulcra-ingest-webhooks
description: "Instructions for setting up webhooks to allow external sources to push data directly to Fulcra without polling."
---

# Fulcra Webhooks

Webhooks are dedicated, secure URLs that allow 3rd-party services (like GitHub, Zapier, IFTTT, or custom scripts) to push data directly into Fulcra, bypassing the need for an agent to constantly poll or run cron jobs.

## When to Use Webhooks
If a data source supports "Webhooks" or HTTP POST notifications (e.g., GitHub issue updates, Stripe payments, Slack messages), you should configure a webhook rather than setting up a local heartbeat/cron polling loop.

## Creating a Webhook
You can generate a new webhook URL via the Fulcra CLI. 
When creating the link, you define where the incoming data should be stored in the user's File Store.

```bash
# Create a webhook that saves incoming payloads to a specific path
...
For direct data ingestion, Fulcra webhooks have the following form:
`https://fulcra.io/e/<LINK ID>?value=<VALUE>&note=<NOTE>&tag=<TAG 1 UUID>&tag=<TAG 2 UUID>`

Notice how the webhook query parameters relate directly to the annotation recording data and metadata:
...
1. Provide the user the Webhook URL.
...
**Note:** If multiple pushes happen rapidly, the file at that path will be overwritten with the latest payload. Because Fulcra's File Store is versioned, previous payloads are not permanently lost, but typical webhook ingest scripts should process and archive the file quickly to maintain a clear pipeline.