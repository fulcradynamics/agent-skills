...
- **If you are the NEW (destination) agent:**
  You must first ask the user to confirm that the old agent has recently backed itself up using the `fulcra-memory` skill. Ask the user for the exact Fulcra backup path (e.g., `agent/<old-agent-name>/memory/memory.tar.gz`). Once provided, you can download that archive and extract it.
