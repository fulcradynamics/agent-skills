...
3. **Theming & Visualization:**
   ...
   - **Hero Text Legibility:** When styling the hero header, ensure the text overlaid on the image is highly legible. Adapt the technique to fit the theme (e.g., use a frosted glass `backdrop-filter: blur()` block for tech/modern themes, an ambient `radial-gradient` vignette for dark/moody themes, or a stark geometric block background for editorial/clean themes).
